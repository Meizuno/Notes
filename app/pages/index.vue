...
useSeoMeta({ title: 'Graph · Knowledge Base' })
...
// Hand the fetched data to the local refs the simulation reads.
// Done eagerly so the values exist by the time `onMounted` runs.
if (graph.value) {
  nodes.value = graph.value.nodes
  edges.value = graph.value.edges
}

// Aspect-aware centering force. Larger strength → tighter spread on
// that axis. On portrait viewports (mobile) we squeeze horizontally
// so the graph stretches into the available vertical space; on
// landscape we apply a balanced (gentle) pull on both axes.
...
// Iterations to run synchronously before the first render. Trades
// ~30–80ms of CPU at mount for skipping the long visible "drifting
// nodes" animation. Tuned for vaults up to a few hundred notes; if
// graphs grow larger, scale this down or switch to canvas rendering.
...
function startSim() {
  ...
  simulation = forceSimulation<Node>(nodes.value)
    ...
    .alphaDecay(0.05)  // faster decay → shorter visible polish phase

  // Pre-warm: tick to a near-settled layout without rendering. After
  // this, the visible animation only damps the last few % of motion.
  simulation.stop()
  ...
  simulation
    .alpha(0.3)
    .on('tick', () => {
      // d3-force mutates x/y in place; trigger Vue's reactivity so
      // the bindings re-read.
      triggerRef(nodes)
      triggerRef(edges)
      // Auto-fit the camera to the layout while the simulation is
      // settling, so the whole graph stays visible — especially on
      // small screens. Stops as soon as the user touches the camera.
...
const FIT_PADDING = 50  // px of breathing room around the bounding box
...
function fitToViewport() {
  ...
  const k = Math.min(
    ...
    1.5  // allow modest zoom-in when the layout is small, so empty
         // space gets filled rather than left as margins
  )
...
function onWheel(e: WheelEvent) {
  ...
  // Zoom around the cursor: keep the graph point under the mouse fixed.
...
function onPanStart(e: PointerEvent) {
  if ((e.target as Element).closest('[data-node]')) return  // node drag handles itself
  userInteracted.value = true
  panning = true
...
function onNodePointerUp(node: Node, e: PointerEvent) {
  ...
  // Release the pin so physics resumes for this node.
  node.fx = null
  node.fy = null
  // If the pointer barely moved, treat as click → navigate.
...
onMounted(() => {
  ...
  // Data already populated synchronously from useFetch above; just
  // kick the simulation now that we have a measured viewport.
